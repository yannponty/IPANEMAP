...
\begin{enumerate}
	...
	\TODO{Add minimal page and reactivities, including scripts/instructions to compute reactivities.\\ Bruno: non je pense que c'est inutile. Les reactivité sont déja données en supplémentary matérial, pour ce qui est de donner un moyen de les calculer, tout est dans l'article de weeks sur shappemapper qu'on cite. Par contre il faut déposer le script qui permet de traiter les fichier ion-torrent si ce n'est déja fait}
	}
...
\newpage
	...
	\Answer{We understand those reservations but, again, we need to stress that we already  {\bfseries explicitly describe this study as a case study}, and do not make any claim regarding the superiority of a given technology or probe.
	
	We also did reproduce in triplicate all of our SHAPE CE probing experiments (but not our SHAPE-MaP ones due to the relatively high cost and length of those experiments in our wet lab), and observed minor variations at the reactivity level, leaving us to only consider a single representative (average of available reactivities) for each experiment. We include in this revision the reactivities measured over each of the 3 replicates for each condition.
	}
