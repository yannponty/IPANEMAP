...
Following Xu\etal\cite{Xu2012}, we assessed the statistical significance of observed changes in predictive performances using a two-tailed Student $t$-test (unequal variance) with usual significance threshold $\alpha=5\%$. When comparing performances within a given dataset, we used the paired version of the test.
...
\RevB{%
\OurTool{} appears to perform comparably, or even slightly favorably, to \Software{RSample}, reporting more accurate structures than its competitor for roughly two thirds of the tested RNAs, while averaging $80.1\%$ GM. However, the  improvement is only of around 1.5\% on average over the \citet{Hajdin2013} dataset, an improvement that is no longer significant according to a two-tailed paired T-test.
}
