...
Finally, considering the 16 conditions together induced a 80\% MCC value, increasing to 83\% in the absence of cluster \Bull{B}. \Rev{Overall, these results support the notion that the strategy used} by \OurTool{} is able to exploit the complementary information provided by multiple conditions, contributing to better predictions.
	...
	\begin{adjustbox}{max width=\linewidth}
		...
		\end{tabular}
	\end{adjustbox}\\
	\caption{Predictive performances of conditions across clusters.  In each line, a single condition is chosen from each cluster except for cluster \Bull{B}. \Rev{Clusters consisting of a single condition (\Bull{D}, \Bull{F} and \Bull{H}) contribute additional conditions  $ :=\{\OneMSevILUThree, \BzCNMg, \DMSMg\}$.
		The MCC of the predicted structure is reported ('Joint' columns) and compared to the average MCC of mono conditions, in all cases in presence/absence of \NAICE.}
