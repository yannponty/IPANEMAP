...
\ypi{Dans la mesure où l'intro est courte, et étant donné le titre, je ne pense pas que sa présence en Mat Met limite  sur visibilité. En plus, c'est typiquement là où tu le trouves d'habitude dans ce type de papier (cf Rsample).} \bsi{OK je propose la chose suivante:on laisse tt comme ca le math et met, mais on commence les résultats par un mini-chapitre du genre "principles of IPANEMAP" IPANEMAP relies on .. prediction with RNAsubopt ... sampling ... clustering .. evaluation by front pareto.. Its originality relies on ... See material and methods for details }
...
\end{figure*}
As benchmark RNA we used the Lariat capping ribozyme from \textit{Didymium Iridis} (DiLCrz) which 3D structure has been modeled at 3,85 A from X-ray diffraction data~\cite{Meyer2014}. This RNA contains a large diversity of interactions, including helical domains, tertiary pairings, a kissing loop and a pseudoknot (Figure~\ref{fig:didy}).
...
For each set of probing profiles \bsi{c'est ce qui correspondrait chez nous à du DMS Map mg ilu ce qu'il a fait. Tu peux l'insérer la avec notre nomenclature entre paranthèse step ?}, we executed \OurTool{} with default parameters, using a sample size of 1~000 structures. 
